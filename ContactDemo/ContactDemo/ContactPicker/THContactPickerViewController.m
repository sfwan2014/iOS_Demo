...
- (void)viewDidLoad
{
    ...
    barButton = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStyleDone target:self action:@selector(done:)];
...
-(void)getContactsFromAddressBook
{
    ...
    NSArray *words = [kWords componentsSeparatedByString:@","];
    if (addressBook) {
...
//        NSMutableArray *mutableContacts = [NSMutableArray arrayWithCapacity:allContacts.count];
        ...
        for (i = 0; i<[allContacts count]; i++)
        {
            ...
            NSString *fullNamePinyin = [contact chineseToPinyin:fullName];
            for (NSString *word in words) {
                NSMutableArray *array = mutableDic[word];
                if (array == nil) {
                    array = [NSMutableArray array];
                }
                if ([fullNamePinyin hasPrefix:word]) {
                    [array addObject:contact];
                    array = [[array sortedArrayUsingSelector:@selector(compare:)] mutableCopy];
                    [mutableDic setObject:array forKey:word];
                    break;
                }
            }
            //[{[],[],[]}]
//            [mutableContacts addObject:mutableDic];
...
//        NSArray *array = [mutableContacts sortedArrayUsingSelector:@selector(compare:)];
        
        self.contacts = [NSArray arrayWithObject:mutableDic];
...
//    [self.tableView reloadData];
    for (NSDictionary *dic in self.contacts) {
        ...
        for (NSString *key in allKeys) {
            ...
            for (THContact* contact in array)
            {
                [self refreshContact: contact];
            }
        }
...
//    return 1;
    ...
    NSDictionary *dic = [self.filteredContacts lastObject];
    NSArray *allkeys = [dic allKeys];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    THContact *contact = [array objectAtIndex:indexPath.row];
    ...
    UIImage *image;
    if ([self.selectedContacts containsObject:[array objectAtIndex:indexPath.row]]){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    THContact *user = [array objectAtIndex:indexPath.row];
    UIImageView *checkboxImageView = (UIImageView *)[cell viewWithTag:104];
    UIImage *image;
...
- (void)contactPickerTextViewDidChange:(NSString *)textViewText {
    ...
    if ([textViewText isEqualToString:@""]){
        ...
        NSMutableDictionary *dic = [self.contacts lastObject];
        ...
        for (NSString *key in allKeys) {
            NSArray *array = dic[key];
            NSArray *preArray = [array filteredArrayUsingPredicate:predicate];
            if (preArray.count > 0) {
                [newDic setObject:preArray forKey:key];
            }
        }
        self.filteredContacts = @[newDic];
    }
    [self.tableView reloadData];
}
...
- (void)contactPickerDidRemoveContact:(id)contact {
    ...
    NSDictionary *dic = [_contacts lastObject];
    NSArray *allKeys = [dic allKeys];
    allKeys = [allKeys sortedArrayUsingSelector:@selector(compare:)];
    for (NSString *key in allKeys) {
        NSArray *array = dic[key];
        NSUInteger section = [allKeys indexOfObject:key];
        NSUInteger row = [array indexOfObject:contact];
        if (row != NSNotFound) {
