...
//
//此文件是google开源工程，功能是为了在mac osx及iOS系统上异步网络I/O
//此文件代码除BUG外，不做改动
				...
				error:(NSError **)errPtr
{
	...
	{
		SCOV_TEST();
		const char* message = "Attempting to connect without a delegate. Set a delegate first.";
		[NSException raise:AsyncSocketException format:@"%s",message];
	}

	if(theSocket != NULL || theSocket6 != NULL)
	{
		SCOV_TEST();
		const char *message = "Attempting to connect while connected or accepting connections. Disconnect first.";
		[NSException raise:AsyncSocketException format:@"%s",message];
	}
