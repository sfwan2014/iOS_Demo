...
//    NSArray *downImages = @[@"down_red@2x.png", @"down_white@2x.png"];
    ...
    for (int i = 0; i < items.count; i++) {
        ...
        ITTArrowView *arrowView = [[ITTArrowView alloc] initWithFrame:CGRectMake(itemWidth-30, 5, 30, 30)];
...
-(void)setSelectedIndex:(int)selectedIndex
{
    ...
    for (int i = 0; i < allItems.count; i++) {
        ...
        if (i == selectedIndex) {
            ...
            arrowView.isSelected = YES;
            indexView.frame = CGRectMake(0, 39, itemWidth, 5);
            indexView.image = [UIImage imageNamed:@"index_press.png"];
        } else {
            titleLabel.textColor = [UIColor blackColor];
            arrowView.isSelected = NO;
            indexView.image = [UIImage imageNamed:@"index.png"];
            indexView.frame = CGRectMake(0, 40, itemWidth, 3);
        }
    }
}
